...
/**
 * Internal intermediary structure derived from FCustomShaderCameraModel by the game thread
 * to hand to the render thread.
 */
struct FCompiledCameraModel
{
	/** Orignal camera model that has generated this compiled model. */
	FCustomShaderCameraModel OriginalCameraModel;
...
static FVector2D LensUndistortViewportUVIntoViewSpace(
	const FCustomShaderCameraModel& CameraModel,
...
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationVS, TEXT("/Plugin/Private/MyShader.usf"), TEXT("MainVS"), SF_Vertex)
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationPS, TEXT("/Plugin/Private/MyShader.usf"), TEXT("MainPS"), SF_Pixel)
...
FVector2D FCustomShaderCameraModel::UndistortNormalizedViewPosition(FVector2D EngineV) const
{
...
float FCustomShaderCameraModel::GetUndistortOverscanFactor(
	float DistortedHorizontalFOV, float DistortedAspectRatio) const
{
	...
	if (*this == FCustomShaderCameraModel())
	{
		return 1.0f;
	}
...
void FCustomShaderCameraModel::DrawUVDisplacementToRenderTarget(
	UWorld* World,
